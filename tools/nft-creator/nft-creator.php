...
$can_log = file_exists($log_dir) && is_dir($log_dir) && is_writable($log_dir) && (file_exists($log_file) ? is_writable($log_file) : true);
if ($can_log) {
    log_message("nft_creator: bootstrap.php loaded", 'nft_creator_log.txt', 'DEBUG');
}

// Cache directory and file
$cache_dir = defined('NFT_CREATOR_PATH') ? NFT_CREATOR_PATH . 'cache/' : dirname(__FILE__) . '/cache/';
...
if (!file_exists($cache_dir) || !is_writable($cache_dir) || !file_exists($cache_file) || !is_writable($cache_file)) {
    if ($can_log) {
...
<div class="nft-creator">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['creatorAddress'])) {
        ...
        if (time() - $rate_limit_time > 60) {
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Reset rate limit for IP=$ip, count=1\n", FILE_APPEND);
            }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Rate limit exceeded for IP=$ip, count=$rate_limit_count\n", FILE_APPEND);
            }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Incremented rate limit for IP=$ip, count=" . $_SESSION[$rate_limit_key]['count'] . "\n", FILE_APPEND);
            }
        }
    }
    ...
    if (!$rate_limit_exceeded) {
        if ($can_log) {
            ...
            @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Rendering form\n", FILE_APPEND);
        }
        ?>
        ...
        try {
            ...
            if (isset($_POST['csrf_token']) && function_exists('validate_csrf_token') && !validate_csrf_token($_POST['csrf_token'])) {
                if ($can_log) {
                    ...
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Invalid CSRF token\n", FILE_APPEND);
                }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Validating creatorAddress=$creatorAddress\n", FILE_APPEND);
            }
            if (!preg_match('/^[1-9A-HJ-NP-Za-km-z]{32,44}$/', $creatorAddress)) {
                if ($can_log) {
                    ...
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Invalid Creator Address format\n", FILE_APPEND);
                }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Cache cleared for creatorAddress=$creatorAddress\n", FILE_APPEND);
            }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Calling getAssetsByCreator API for creatorAddress=$creatorAddress\n", FILE_APPEND);
            }
            ...
            if (isset($response['error'])) {
                ...
                if ($can_log) {
                    ...
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] API error: $error_msg\n", FILE_APPEND);
                }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] API returned " . count($items) . " items\n", FILE_APPEND);
            }
            if (empty($items)) {
                if ($can_log) {
                    log_message("nft_creator: Empty result for creatorAddress=$creatorAddress", 'nft_creator_log.txt', 'ERROR');
                } else {
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Empty result for creatorAddress=$creatorAddress\n", FILE_APPEND);
                }
            ...
            foreach ($assets as $index => $asset) {
                ...
                if ($can_log) {
                    ...
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [DEBUG] Asset $index: ID={$asset['id'] ?? 'N/A'}, Name=" . ($asset['content']['metadata']['name'] ?? ($asset['name'] ?? 'N/A')) . "\n", FILE_APPEND);
                }
            }
            ...
            if ($fp && flock($fp, LOCK_EX)) {
                if (!@file_put_contents($cache_file, json_encode($cache_data, JSON_PRETTY_PRINT))) {
                    if ($can_log) {
                        ...
                        @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Failed to write to cache file\n", FILE_APPEND);
                    }
                ...
                if ($can_log) {
                    ...
                    @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Failed to lock cache file\n", FILE_APPEND);
                }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Cache updated for creatorAddress=$creatorAddress\n", FILE_APPEND);
            }
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Output length: " . strlen($output) . "\n", FILE_APPEND);
            }
            echo $output;
        } catch (Exception $e) {
            ...
            if ($can_log) {
                ...
                @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [ERROR] Exception: $error_msg, File: " . htmlspecialchars($e->getFile()) . ", Line: " . $e->getLine() . "\n", FILE_APPEND);
            }
    ...
    if ($can_log) {
        ...
        @file_put_contents('/tmp/nft_creator_debug_log.txt', "[" . date('Y-m-d H:i:s') . "] [INFO] Script ended\n", FILE_APPEND);
    }
    ?>
