        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['creatorAddress']) && !$rate_limit_exceeded) {
            try {
                ...
                if (!empty($items)) {
                    ...
                    log_message("nft_creator: First item keys: " . implode(', ', $item_keys), 'nft_creator_log.txt', 'DEBUG');
                ...
                foreach ($assets as $index => $asset) {
                    ...
                    $name = $asset['content']['metadata']['name'] ?? ($asset['metadata']['name'] ?? ($asset['name'] ?? 'Unnamed NFT'));
                    $image = $asset['content']['links']['image'] ?? ($asset['image_url'] ?? ($asset['image'] ?? ''));
                    $formatted_data[] = [
