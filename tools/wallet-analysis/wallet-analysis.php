    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['walletAddress']) && !$rate_limit_exceeded) {
        try {
            ...
            if (!$cache_valid || !$names_cache_valid) {
                ...
                if (!$names_cache_valid) {
                    ...
                    if (isset($names_data['error']) || empty($names_data['domainNames'])) {
                        ...
                        foreach ($domains as $name) {
                            ...
                            $formatted_data['sol_domains'][] = [
                                'domain' => $domain_name,
                                'mint' => 'N/A' // Helius không trả mint address
                            ];
                        }
                    ...
                    if (flock($fp, LOCK_EX)) {
                        if (file_put_contents($names_cache_file, json_encode($names_cache_data, JSON_PRETTY_PRINT)) === false) {
                            log_message("wallet_analysis: Failed to write names cache to $names_cache_file", 'wallet_analysis_log.txt', 'ERROR');
                            flock($fp, LOCK_UN);
                            fclose($fp);
                            throw new Exception('Failed to save names cache data');
                        }
                        flock($fp, LOCK_UN);
                    } else {
                        log_message("wallet_analysis: Failed to lock names cache file $names_cache_file", 'wallet_analysis_log.txt', 'ERROR');
                        fclose($fp);
                ...
                if (flock($fp, LOCK_EX)) {
                    if (file_put_contents($cache_file, json_encode($cache_data, JSON_PRETTY_PRINT)) === false) {
                        log_message("wallet_analysis: Failed to update cache with sol_domains to $cache_file", 'wallet_analysis_log.txt', 'ERROR');
                        flock($fp, LOCK_UN);
                        fclose($fp);
                        throw new Exception('Failed to update cache data');
                    }
                    flock($fp, LOCK_UN);
                } else {
                    log_message("wallet_analysis: Failed to lock cache file $cache_file for update", 'wallet_analysis_log.txt', 'ERROR');
                    fclose($fp);
                    throw new Exception('Failed to lock cache file');
                }
                fclose($fp);
                    ...
                    <div class="result-card">
                        <div class="result-item">
                            ...
                            <p class="wallet-address">
                                ...
                                <i class="fas fa-copy copy-icon" title="Copy full address" data-address="<?php echo htmlspecialchars($formatted_data['wallet_address']); ?>"></i>
                            </p>
                            <h3>SOL Balance</h3>
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <i class="fas fa-copy copy-icon" title="Copy full address" data-address="<?php echo htmlspecialchars($token['mint']); ?>"></i>
                                </td>
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <i class="fas fa-copy copy-icon" title="Copy full address" data-address="<?php echo htmlspecialchars($nft['mint']); ?>"></i>
                                </td>
                                <td>
                                    <?php if ($nft['collection'] !== 'N/A' && preg_match('/^[1-9A-HJ-NP-Za-km-z]{32,44}$/', $nft['collection'])): ?>
                                        ...
                                        <i class="fas fa-copy copy-icon" title="Copy full address" data-address="<?php echo htmlspecialchars($nft['collection']); ?>"></i>
                                    <?php else: ?>
                                        <span>N/A</span>
                                    <?php endif; ?>
                                </td>
