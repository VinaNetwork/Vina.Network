...
// Kết quả sẽ được render bởi token.php, nft.php, domain.php tùy theo tab
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['walletAddress']) && !$rate_limit_exceeded) {
    try {
        ...
        if (!$cache_valid) {
            ...
            $formatted_data['sol_balance'] = isset($result['nativeBalance']['lamports']) ? $result['nativeBalance']['lamports'] / 1e9 : 0.0;
            $formatted_data['sol_price_usd'] = $result['nativeBalance']['total_price'] ?? 0.0;
            ...
            foreach ($result['items'] as $item) {
                if ($item['interface'] === 'FungibleToken') {
                    $formatted_data['tokens'][] = [
                        ...
                        'price_usd' => $item['token_info']['price_info']['total_price'] ?? 0.0
                    ];
...
?>
        <div class="tools-result wallet-analysis-result">
            ...
            <div class="wallet-tabs">
                <a href="?tab=token" class="wallet-tab-link <?php echo (!isset($_GET['tab']) || $_GET['tab'] === 'token') ? 'active' : ''; ?>" data-tab="token"><i class="fas fa-coins"></i> Tokens</a>
                <a href="?tab=nft" class="wallet-tab-link <?php echo (isset($_GET['tab']) && $_GET['tab'] === 'nft') ? 'active' : ''; ?>" data-tab="nft"><i class="fas fa-image"></i> NFTs</a>
                <a href="?tab=domain" class="wallet-tab-link <?php echo (isset($_GET['tab']) && $_GET['tab'] === 'domain') ? 'active' : ''; ?>" data-tab="domain"><i class="fas fa-globe"></i> Domains</a>
            </div>

            <div class="wallet-tab-content">
                <?php
                $tab = $_GET['tab'] ?? 'token';
                if (!in_array($tab, ['token', 'nft', 'domain'])) $tab = 'token';
...
document.addEventListener("DOMContentLoaded", () => {
    const addSolscanLink = (selector) => {
        document.querySelectorAll(selector).forEach(el => {
            const addr = el.textContent.trim();
            if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr)) {
                el.innerHTML = `<a href="https://solscan.io/token/${addr}" target="_blank">${addr.slice(0, 4)}...${addr.slice(-4)}</a>`;
            }
        });
    };
    addSolscanLink('.token-address');
    addSolscanLink('.mint-address');
    addSolscanLink('.collection-address');
});
</script>
