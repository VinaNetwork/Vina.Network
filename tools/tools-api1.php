<?php
// File: tools/tools-api1.php
// Description: Universal wrapper to call Helius RPC API on Solana (updated for nft-events).
...
function callAPI($endpoint, $params = [], $method = 'POST') {
    ...
    do {
        ...
        } elseif ($endpoint === 'nft-events') {
            // New NFT transaction endpoint
            $url = "https://api.helius.xyz/v1/nft-events?api-key=$api_key";
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_POST, 1);
            $postData = json_encode($params);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);
            log_message("api-helper: POST request - URL: " . str_replace($api_key, '****', $url) . ", Params: " . substr($postData, 0, 100) . "...", 'api_log.txt');
        } else {
            // JSON-RPC method
        ...
        if ($data === null) {
            log_message("api-error: Failed to parse API response as JSON. URL: " . str_replace($api_key, '****', $url), 'api_log.txt', 'ERROR');
            return ['error' => 'Failed to parse API response as JSON.'];
        }

        if (isset($data['error'])) {
            $errorMessage = is_array($data['error']) && isset($data['error']['message']) ? $data['error']['message'] : json_encode($data['error']);
            log_message("api-error: API error - Message: $errorMessage", 'api_log.txt', 'ERROR');
            return ['error' => $errorMessage];
        }

        log_message("api-success: API success - Endpoint: $endpoint, URL: " . str_replace($api_key, '****', $url), 'api_log.txt');
        return $data;
