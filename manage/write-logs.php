...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    log_message("Invalid request method: {$_SERVER['REQUEST_METHOD']}, IP=" . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), 'manage-users.log', 'accounts', 'ERROR');
...
if ($authToken !== JWT_SECRET) {
    log_message("Invalid or missing X-Auth-Token, IP=" . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), 'manage-users.log', 'accounts', 'ERROR');
...
if (!$data || !isset($data['message'], $data['log_type'], $data['log_file'], $data['module'])) {
    log_message("Invalid log data: Missing required fields, IP=" . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), 'manage-users.log', 'accounts', 'ERROR');
    http_response_code(400);
