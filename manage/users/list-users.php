<?php
// ============================================================================
// File: manage/users/list-users.php
// Description: List users of Vina Network page.
...
log_message("Attempting to access admin page, public_key: $short_public_key, session_role: " . ($_SESSION['role'] ?? 'Not set'), 'manage-users.log', 'accounts', 'DEBUG');

// Database connection
try {
    $pdo = get_db_connection();
    log_message("Database connection successful for admin page", 'manage-users.log', 'accounts', 'INFO');
} catch (PDOException $e) {
    log_message("Database connection failed: {$e->getMessage()}", 'manage-users.log', 'accounts', 'ERROR');
...
if ($public_key) {
    try {
        ...
        if ($account && $account['is_active'] && $account['role'] === 'admin') {
            ...
            log_message("Role verified from database for public_key: $short_public_key, role: admin", 'manage-users.log', 'accounts', 'INFO');
        } else {
            log_message("Unauthorized access attempt to admin page, public_key: $short_public_key, role: " . ($account['role'] ?? 'Not found') . ", is_active: " . ($account['is_active'] ?? 'Not found'), 'manage-users.log', 'accounts', 'ERROR');
            header('Location: /acc/connect-p?error=Unauthorized access');
            exit;
        }
    } catch (PDOException $e) {
        log_message("Database query failed for role check: {$e->getMessage()}, public_key: $short_public_key", 'manage-users.log', 'accounts', 'ERROR');
        header('Location: /acc/connect-p?error=Database error');
        exit;
    }
} else {
    log_message("Unauthorized access attempt to admin page, public_key: $short_public_key, session_role: Not set, IP=" . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), 'manage-users.log', 'accounts', 'ERROR');
    header('Location: /acc/connect-p?error=Unauthorized access');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['public_key'], $_POST['action'])) {
    ...
    if ($target_public_key === $public_key) {
        log_message("Admin attempted to modify own account: public_key=$target_short_public_key", 'manage-users.log', 'accounts', 'ERROR');
        ...
        try {
            ...
            log_message("Account $target_short_public_key " . ($action === 'lock' ? 'locked' : 'unlocked') . " by admin $short_public_key", 'manage-users.log', 'accounts', 'INFO');
            $success = "Account $target_short_public_key has been " . ($action === 'lock' ? 'locked' : 'unlocked') . ".";
        } catch (PDOException $e) {
            log_message("Failed to update account $target_short_public_key: {$e->getMessage()}", 'manage-users.log', 'accounts', 'ERROR');
...
try {
    ...
    log_message("Failed to fetch accounts: {$e->getMessage()}", 'manage-users.log', 'accounts', 'ERROR');
...
$page_css = ['/manage/users/list-users.css'];
?>
...
<script src="/manage/users/list-users.js"></script>
</body>
</html>
<?php ob_end_flush(); ?>
