<?php
// config.php
// Yêu cầu bắt buộc muốn include file config.php thì phải có VINANETWORK_STATUS
if (!defined('VINANETWORK_STATUS')) {
    http_response_code(403);
    exit('No direct script access allowed!');
}

// Định nghĩa hằng số cấu hình
if (!defined('CONFIG_PATH')) {
    define('CONFIG_PATH', '/var/www/vinanetwork/public_html/config/config.php');
}
if (!defined('ERROR_LOG_PATH')) {
    define('ERROR_LOG_PATH', '/var/www/vinanetwork/public_html/tools/error_log.txt');
}
if (!defined('DEBUG_LOG_PATH')) {
    define('DEBUG_LOG_PATH', '/var/www/vinanetwork/public_html/tools/debug_log.txt');
}
if (!defined('HELIUS_API_KEY')) {
    define('HELIUS_API_KEY', '8eb75cd9-015a-4e24-9de2-5be9ee0f1c63'); // Thay bằng key thực tế
}
if (!defined('BASE_PATH')) {
    define('BASE_PATH', '/var/www/vinanetwork/public_html/');
}
?>
