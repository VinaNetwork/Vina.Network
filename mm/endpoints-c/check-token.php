...
log_message("Parameters received: token_mint=$token_mint, network=$network, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'), 'make-market.log', 'make-market', 'INFO');
...
try {
    ...
    log_message(
        "Response from Jupiter API: url=$url, status=$http_code, response=" . ($response ? substr($response, 0, 1000) . (strlen($response) > 1000 ? '...' : '') : 'none') . ", curl_error=$curl_error",
