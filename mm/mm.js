...
async function ensureValidCSRFToken() {
    let csrfToken = getCookie('csrf_token_cookie');
    let expiresAt = localStorage.getItem('csrf_expires_at');
    
    if (!csrfToken || !expiresAt || Date.now() / 1000 >= parseInt(expiresAt) - 300) {
        try {
            const { token, expires_at } = await refreshCSRFToken();
            csrfToken = token;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('makeMarketForm').addEventListener('submit', async (e) => {
        ...
        let csrfToken;
        try {
            csrfToken = await ensureValidCSRFToken();
