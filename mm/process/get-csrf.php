...
if (defined('ENVIRONMENT') && ENVIRONMENT === 'development') {
    ...
    log_message("get-csrf.php: Request received, method=$request_method, uri=$request_uri, session_id=$session_id, cookies=$cookies, headers=" . json_encode($headers) . ", CSRF_TOKEN: $csrf_token", 'make-market.log', 'make-market', 'DEBUG', $log_context);
}

// Kiểm tra phương thức GET
if ($request_method !== 'GET') {
    log_message("Invalid request method: $request_method, uri=$request_uri, session_id=$session_id, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'), 'make-market.log', 'make-market', 'ERROR', $log_context);
...
if ($csrf_token === false) {
    ...
    log_message("Failed to generate CSRF token, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR', $log_context);
    header('Content-Type: application/json');
    http_response_code(500);
...
if (!set_csrf_cookie()) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message("Failed to set CSRF cookie, user_id=$user_id", 'make-market.log', 'make-market', 'WARNING', $log_context);
}

// Trả về CSRF token
log_message("CSRF token sent: $csrf_token, session_id=$session_id, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'), 'make-market.log', 'make-market', 'INFO', $log_context);
header('Content-Type: application/json');
echo json_encode([
    'status' => 'success',
