...
if (!isset($_SERVER['HTTP_X_REQUESTED_WITH']) || $_SERVER['HTTP_X_REQUESTED_WITH'] !== 'XMLHttpRequest') {
    log_message("Non-AJAX request rejected in get-decimals.php", 'make-market.log', 'make-market', 'ERROR');
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    log_message("Invalid request method in get-decimals.php: {$_SERVER['REQUEST_METHOD']}", 'make-market.log', 'make-market', 'ERROR');
...
if (empty($token_mint) || !preg_match('/^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{32,44}$/', $token_mint)) {
    log_message("Invalid token mint in get-decimals.php: $token_mint", 'make-market.log', 'make-market', 'ERROR');
...
if (!in_array($network, ['mainnet', 'testnet', 'devnet'])) {
    log_message("Invalid network in get-decimals.php: $network", 'make-market.log', 'make-market', 'ERROR');
    http_response_code(400);
    echo json_encode(['status' => 'error', 'message' => 'Invalid network']);
    exit;
}

log_message("Parameters received: token_mint=$token_mint, network=$network, session_id=" . (session_id() ?: 'none'), 'make-market.log', 'make-market', 'INFO');

try {
    // Get database connection
    $pdo = get_db_connection();
    if (!$pdo) {
        log_message("Failed to connect to database in get-decimals.php", 'make-market.log', 'make-market', 'ERROR');
    ...
    if (!$row) {
        log_message("No decimals found in make_market table for token_mint=$token_mint, network=$network, user_id={$_SESSION['user_id']}, using default=9", 'make-market.log', 'make-market', 'INFO');
    ...
    log_message("Failed to retrieve decimals from make_market table: {$e->getMessage()}, token_mint=$token_mint, network=$network, user_id={$_SESSION['user_id']}, session_id=" . (session_id() ?: 'none'), 'make-market.log', 'make-market', 'ERROR');
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Error retrieving decimals: ' . $e->getMessage()]);
    exit;
}
?>
