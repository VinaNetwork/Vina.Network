...
if (defined('ENVIRONMENT') && ENVIRONMENT === 'development') {
    log_message(
        "swap.php: Request received, method=$request_method, uri=$request_uri, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined') . ", session_id=$session_id, cookies=$cookies, headers=" . json_encode($headers),
...
if ($transaction_id <= 0 || !is_array($swap_transactions) || !is_array($sub_transaction_ids) || count($swap_transactions) !== count($sub_transaction_ids) || !in_array($client_network, ['mainnet', 'devnet'])) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Invalid input: transaction_id=$transaction_id, swap_transactions=" . json_encode($swap_transactions, JSON_UNESCAPED_SLASHES) . ", sub_transaction_ids=" . json_encode($sub_transaction_ids) . ", client_network=$client_network, user_id=$user_id",
...
if ($client_network !== SOLANA_NETWORK) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Network mismatch: client_network=$client_network, server_network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined') . ", user_id=$user_id",
        'process.log', 'make-market', 'ERROR', $log_context
    );
    header('Content-Type: application/json');
    http_response_code(400);
...
if (empty(RPC_ENDPOINT)) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "RPC_ENDPOINT is empty, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined') . ", user_id=$user_id",
        'process.log', 'make-market', 'ERROR', $log_context
    );
    header('Content-Type: application/json');
    http_response_code(500);
...
try {
    ...
    if (!$transaction) {
        $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
        log_message(
            "Transaction not found, unauthorized, or network mismatch: ID=$transaction_id, user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        header('Content-Type: application/json');
        http_response_code(403);
    ...
    log_message(
        "Transaction fetched: ID=$transaction_id, user_id=$user_id, public_key={$transaction['public_key']}, token_mint={$transaction['token_mint']}, sol_amount={$transaction['sol_amount']}, trade_direction={$transaction['trade_direction']}, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
        'process.log', 'make-market', 'INFO', $log_context
    );
} catch (PDOException $e) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Database query failed: " . $e->getMessage() . ", user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
        'process.log', 'make-market', 'ERROR', $log_context
    );
    header('Content-Type: application/json');
    http_response_code(500);
...
try {
    if (!defined('JWT_SECRET') || empty(JWT_SECRET)) {
        $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
        log_message(
            "JWT_SECRET is not defined or empty, user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        header('Content-Type: application/json');
        http_response_code(500);
    ...
    if ($private_key === false) {
        $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
        log_message(
            "Failed to decrypt private key, user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        header('Content-Type: application/json');
        http_response_code(500);
    ...
    log_message(
        "Private key decrypted successfully, user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
        'process.log', 'make-market', 'INFO', $log_context
    );
} catch (Exception $e) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Private key decryption failed: " . $e->getMessage() . ", user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
        'process.log', 'make-market', 'ERROR', $log_context
    );
    header('Content-Type: application/json');
    http_response_code(500);
...
foreach ($swap_transactions as $index => $swap) {
    ...
    if ($sub_transaction_id === 0 || empty($swap_transaction)) {
        log_message(
            "Invalid sub-transaction ID or swap transaction, direction=$direction, loop=$loop, batch_index=$batch_index, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        $results[] = [
            'loop' => $loop,
    ...
    try {
        ...
        log_message(
            "Private key decoded successfully, length=" . strlen($decoded_private_key) . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'INFO', $log_context
        );
    } catch (Exception $e) {
        log_message(
            "Failed to decode private key for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        try {
            ...
            log_message(
                "Failed to update sub-transaction status: " . $e2->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                'process.log', 'make-market', 'ERROR', $log_context
            );
        }
        $results[] = [
    ...
    try {
        ...
        log_message(
            "Keypair created successfully for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'INFO', $log_context
        );
    } catch (Exception $e) {
        log_message(
            "Failed to create keypair for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    if ($derivedPublicKey !== $transaction['public_key']) {
        log_message(
            "Public key mismatch for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index: derived=$derivedPublicKey, stored={$transaction['public_key']}, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    try {
        ...
        log_message(
            "Transaction decoded successfully for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'INFO', $log_context
        );
    } catch (Exception $e) {
        log_message(
            "Failed to decode transaction for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    try {
        ...
        log_message(
            "Transaction signed successfully for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'INFO', $log_context
        );
    } catch (Exception $e) {
        log_message(
            "Failed to sign transaction for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
            'process.log', 'make-market', 'ERROR', $log_context
        );
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
        ...
        try {
            $serialized_tx = $transactionObj->serialize();
            log_message(
                "Serialized transaction for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, serialized_length=" . strlen($serialized_tx) . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                'process.log', 'make-market', 'DEBUG', $log_context
            );
            $txid = $connection->sendRawTransaction($serialized_tx);
            log_message(
                "Swap transaction sent for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, txid=$txid, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined') . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                'process.log', 'make-market', 'INFO', $log_context
            );
            try {
                ...
                log_message(
                    "Sub-transaction status updated: ID=$sub_transaction_id, status=success, txid=$txid, direction=$direction, loop=$loop, batch_index=$batch_index, user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                    'process.log', 'make-market', 'INFO', $log_context
                );
            } catch (PDOException $e) {
                log_message(
                    "Failed to update sub-transaction status: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                    'process.log', 'make-market', 'ERROR', $log_context
                );
            }
            $results[] = [
            ...
            log_message(
                "Failed to send transaction for sub-transaction ID=$sub_transaction_id, direction=$direction, loop=$loop, batch_index=$batch_index, attempt=$attempt/$maxRetries: " . $e->getMessage() . ", user_id=" . (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none'),
                'process.log', 'make-market', 'ERROR', $log_context
            );
...
try {
    ...
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Main transaction status updated: ID=$transaction_id, status=$overall_status, success_count=$success_count, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined') . ", user_id=$user_id",
        'process.log', 'make-market', 'INFO', $log_context
    );
} catch (PDOException $e) {
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 'none';
    log_message(
        "Failed to update main transaction status: " . $e->getMessage() . ", user_id=$user_id, network=" . (defined('SOLANA_NETWORK') ? SOLANA_NETWORK : 'undefined'),
        'process.log', 'make-market', 'ERROR', $log_context
    );
    header('Content-Type: application/json');
    http_response_code(500);
