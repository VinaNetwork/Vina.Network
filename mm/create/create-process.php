<?php
// ============================================================================
// File: mm/create/create-process.php
...
// Log request info
log_message("create-process.php: Script started, REQUEST_METHOD: {$_SERVER['REQUEST_METHOD']}, REQUEST_URI: {$_SERVER['REQUEST_URI']}", 'make-market.log', 'make-market', 'DEBUG');
    ...
    try {
        ...
        if (!$errorMessage) {
            ...
            try {
                ...
                $_SERVER['REQUEST_URI'] = '/mm/create/check-decimals.php';
                ...
                // Call decimals.php
                require_once $root_path . 'mm/create/check-decimals.php';
        ...
        if (!$errorMessage && !$skipBalanceCheck && isValidTradeDirection($tradeDirection, $solAmount, $tokenAmount)) {
            ...
            try {
                ...
                $_SERVER['REQUEST_METHOD'] = 'POST';
                $_SERVER['REQUEST_URI'] = '/mm/create/check-balance.php';
                $_SERVER['HTTP_X_REQUESTED_WITH'] = 'XMLHttpRequest';
                $_SERVER['HTTP_X_CSRF_TOKEN'] = $csrf_token;
                $_COOKIE['PHPSESSID'] = session_id();
                ...
                require_once $root_path . 'mm/create/check-balance.php';
                $response = ob_get_clean();
...
$page_url = BASE_URL . "mm/create-process";
$page_css = ['/mm/create/create-process.css'];
...
<script defer src="/mm/create/create-process.js"></script>
</body>
</html>
<?php ob_end_flush(); ?>
