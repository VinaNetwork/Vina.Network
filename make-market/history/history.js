...
// Description: JavaScript file for UI interactions on Transaction History page
...
async function refreshTransactionHistory(page = 1, per_page = 10) {
    ...
    try {
        ...
        const response = await fetch(`/make-market/history/?page=${page}&per_page=${per_page}`, {
        ...
        document.querySelectorAll('.continue-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                window.location.href = `/make-market/?transactionId=${transactionId}`;
            });
        });
