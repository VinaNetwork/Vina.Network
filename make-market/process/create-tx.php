...
log_request_info('create-tx.php');
...
try {
    ...
    if (count($sub_transactions) !== $expected_total) {
        log_message("Sub-transaction count mismatch: expected=$expected_total, received=" . count($sub_transactions) . ", user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        http_response_code(400);
...
try {
    ...
    log_message("Created " . count($sub_transaction_ids) . " sub-transactions for transaction ID=$transaction_id, IDs: " . implode(',', $sub_transaction_ids) . ", user_id=$user_id", 'make-market.log', 'make-market', 'INFO');
} catch (PDOException $e) {
    log_message("Failed to create sub-transactions: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
    http_response_code(500);
