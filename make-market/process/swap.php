...
if ($transaction_id <= 0 || !$swap_transaction) {
    ...
    echo json_encode(['status' => 'error', 'message' => 'Dữ liệu giao dịch không hợp lệ']);
    exit;
}
...
try {
    ...
    if (!$transaction || $transaction['user_id'] != ($_SESSION['user_id'] ?? 0)) {
        ...
        http_response_code(403);
        echo json_encode(['status' => 'error', 'message' => 'Giao dịch không tồn tại hoặc không được phép']);
        exit;
    }
...
try {
    if (!defined('JWT_SECRET') || empty(JWT_SECRET)) {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi cấu hình server']);
        exit;
    }
    ...
    if ($private_key === false) {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi giải mã khóa riêng']);
        exit;
    }
...
try {
    ...
    try {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi xử lý khóa riêng']);
        exit;
    }
    ...
    try {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi tạo cặp khóa']);
        exit;
    }
    ...
    if ($derivedPublicKey !== $transaction['public_key']) {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Địa chỉ ví không khớp']);
        exit;
    }
    ...
    try {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi giải mã giao dịch']);
        exit;
    }
    ...
    try {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi ký giao dịch']);
        exit;
    }
    ...
    try {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi gửi giao dịch']);
        exit;
    }
    ...
    try {
        ...
        echo json_encode(['status' => 'success', 'message' => 'Giao dịch swap hoàn tất thành công', 'txid' => $txid]);
    } catch (PDOException $e) {
        log_message("Failed to update transaction status: {$e->getMessage()}", 'make-market.log', 'make-market', 'ERROR');
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Lỗi cập nhật trạng thái giao dịch']);
        exit;
    }
} catch (Exception $e) {
    ...
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Lỗi thực hiện giao dịch swap']);
    exit;
}
?>
