...
setup_cors_headers($csp_base, 'POST');
check_ajax_request();
$user_id = check_authenticated_user();
log_request_info('swap.php');
...
if ($transaction_id <= 0 || !is_array($swap_transactions) || !is_array($sub_transaction_ids) || count($swap_transactions) !== count($sub_transaction_ids) || !in_array($client_network, ['testnet', 'mainnet'])) {
    log_message("Invalid input: transaction_id=$transaction_id, swap_transactions=" . json_encode($swap_transactions) . ", sub_transaction_ids=" . json_encode($sub_transaction_ids) . ", client_network=$client_network, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
...
if ($client_network !== SOLANA_NETWORK) {
    log_message("Network mismatch: client_network=$client_network, server_network=" . SOLANA_NETWORK . ", user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
    http_response_code(400);
...
try {
    $stmt = $pdo->prepare("SELECT user_id, public_key, token_mint, sol_amount, token_amount, trade_direction, private_key FROM make_market WHERE id = ? AND user_id = ?");
...
try {
    if (!defined('JWT_SECRET') || empty(JWT_SECRET)) {
        log_message("JWT_SECRET is not defined or empty, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        http_response_code(500);
    ...
    if ($private_key === false) {
        log_message("Failed to decrypt private key, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Failed to decrypt private key'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    log_message("Private key decrypted successfully, user_id=$user_id", 'make-market.log', 'make-market', 'INFO');
} catch (Exception $e) {
    log_message("Private key decryption failed: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Failed to decrypt private key'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
foreach ($swap_transactions as $index => $swap) {
    ...
    if ($sub_transaction_id === 0 || empty($swap_transaction)) {
        log_message("Invalid sub-transaction ID or swap transaction for index=$index, direction=$direction, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        $results[] = [
            'loop' => $swap['loop'] ?? 1,
    ...
    try {
        ...
        log_message("Failed to decode private key for sub-transaction ID=$sub_transaction_id, direction=$direction: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        try {
            ...
            log_message("Failed to update sub-transaction status: {$e2->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        }
        $results[] = [
            'loop' => $swap['loop'] ?? 1,
            'batch_index' => $swap['batch_index'] ?? 1,
    ...
    try {
        ...
        log_message("Failed to create keypair for sub-transaction ID=$sub_transaction_id, direction=$direction: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    if ($derivedPublicKey !== $transaction['public_key']) {
        log_message("Public key mismatch for sub-transaction ID=$sub_transaction_id, direction=$direction: derived=$derivedPublicKey, stored={$transaction['public_key']}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    try {
        ...
        log_message("Failed to decode transaction for sub-transaction ID=$sub_transaction_id, direction=$direction: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    try {
        ...
        log_message("Failed to sign transaction for sub-transaction ID=$sub_transaction_id, direction=$direction: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
        try {
            $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ? WHERE id = ?");
    ...
    for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
        try {
            ...
            log_message("Swap transaction sent for sub-transaction ID=$sub_transaction_id, direction=$direction: txid=$txid, network=" . SOLANA_NETWORK . ", user_id=$user_id", 'make-market.log', 'make-market', 'INFO');
            try {
                $stmt = $pdo->prepare("UPDATE make_market_sub SET status = ?, error = ?, txid = ? WHERE id = ?");
                $stmt->execute(['success', null, $txid, $sub_transaction_id]);
                log_message("Sub-transaction status updated: ID=$sub_transaction_id, status=success, txid=$txid, direction=$direction, user_id=$user_id", 'make-market.log', 'make-market', 'INFO');
            } catch (PDOException $e) {
                log_message("Failed to update sub-transaction status: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
            }
            $results[] = [
                'loop' => $swap['loop'] ?? 1,
                'batch_index' => $swap['batch_index'] ?? 1,
            ...
            log_message("Failed to send transaction for sub-transaction ID=$sub_transaction_id, direction=$direction, attempt $attempt/$maxRetries: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
...
try {
    ...
    log_message("Main transaction status updated: ID=$transaction_id, status=$overall_status, success_count=$success_count, network=" . SOLANA_NETWORK . ", user_id=$user_id", 'make-market.log', 'make-market', 'INFO');
} catch (PDOException $e) {
    log_message("Failed to update main transaction status: {$e->getMessage()}, user_id=$user_id", 'make-market.log', 'make-market', 'ERROR');
    http_response_code(500);
