...
try {
    ...
    log_message("Database connection successful (took {$duration}ms)", 'acc_auth.log', 'accounts', 'INFO');
} catch (PDOException $e) {
    $duration = (microtime(true) - $start_time) * 1000;
    log_message("Database connection failed: {$e->getMessage()} (took {$duration}ms)", 'acc_auth.log', 'accounts', 'ERROR');
...
if ($ip_address === 'Unknown') {
    log_message("Failed to retrieve IP address", 'acc_auth.log', 'accounts', 'ERROR');
    header('Content-Type: application/json');
...
try {
    ...
    log_message("Cleaned up old login attempts for IP: $ip_address", 'acc_auth.log', 'accounts', 'INFO');
    ...
    log_message("Checked login attempts for IP: $ip_address, count: $attempt_count", 'acc_auth.log', 'accounts', 'INFO');

    if ($attempt_count >= $rate_limit) {
        log_message("Rate limit exceeded for IP: $ip_address, attempts: $attempt_count", 'acc_auth.log', 'accounts', 'ERROR');
        header('Content-Type: application/json');
    ...
    log_message("Recorded login attempt for IP: $ip_address, attempt count: " . ($attempt_count + 1), 'acc_auth.log', 'accounts', 'INFO');
} catch (PDOException $e) {
    log_message("Rate limiting error for IP: $ip_address, SQL Error: {$e->getMessage()}, Code: {$e->getCode()}", 'acc_auth.log', 'accounts', 'ERROR');
    header('Content-Type: application/json');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['public_key'], $_POST['signature'], $_POST['message'], $_POST['csrf_token'])) {
    ...
    if (!validate_csrf_token($_POST['csrf_token'])) {
        log_message("Invalid CSRF token for login attempt from IP: $ip_address", 'acc_auth.log', 'accounts', 'ERROR');
    ...
    log_message("Received POST: public_key=$short_public_key, message=$message, IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
    log_message("Signature (base64): {$_POST['signature']}", 'acc_auth.log', 'accounts', 'DEBUG');

    if ($signature === false || strlen($signature) !== 64) {
        log_message("Invalid signature: Base64 decode failed or length incorrect (length: " . ($signature === false ? 'decode failed' : strlen($signature)) . "), IP=$ip_address", 'acc_auth.log', 'accounts', 'ERROR');
        echo json_encode(['status' => 'error', 'message' => 'Base64 decode failed or invalid signature']);
        exit;
    }
    log_message("Signature decoded length: " . strlen($signature) . " bytes", 'acc_auth.log', 'accounts', 'DEBUG');
    ...
    try {
        ...
        log_message("Valid timestamp: $timestamp, IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');

        // Check sodium library
        if (!function_exists('sodium_crypto_sign_verify_detached')) {
            throw new Exception("Sodium library is not installed!");
        }
        log_message("Sodium library ready", 'acc_auth.log', 'accounts', 'INFO');
        ...
        log_message("Base58 library ready", 'acc_auth.log', 'accounts', 'INFO');
        ...
        try {
            ...
            $duration = (microtime(true) - $start_time) * 1000;
            log_message("Public key decoded: $short_public_key (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
            log_message("Public key hex: " . bin2hex($public_key_bytes), 'acc_auth.log', 'accounts', 'DEBUG');
        } catch (Exception $e) {
        ...
        $message_raw = $message;
        log_message("Message hex: " . bin2hex($message_raw), 'acc_auth.log', 'accounts', 'DEBUG');
        log_message("Signature hex: " . bin2hex($signature), 'acc_auth.log', 'accounts', 'DEBUG');
        ...
        try {
            ...
            log_message("Signature verified successfully (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
        } catch (Exception $e) {
            $duration = (microtime(true) - $start_time) * 1000;
            log_message("Signature verification error: {$e->getMessage()} (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'ERROR');
            throw $e;
        }
        ...
        try {
            ...
            $account = $stmt->fetch();
            $duration = (microtime(true) - $start_time) * 1000;
            log_message("Account check query: public_key=$short_public_key (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
        } catch (PDOException $e) {
        ...
        log_message("Session ID regenerated: old=$old_session_id, new=$new_session_id, public_key=$short_public_key", 'acc_auth.log', 'accounts', 'INFO');
        ...
        if ($account) {
            ...
            $duration = (microtime(true) - $start_time) * 1000;
            log_message("Login successful: public_key=$short_public_key (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
            ...
            $duration = (microtime(true) - $start_time) * 1000;
            log_message("Registration successful: public_key=$short_public_key (took {$duration}ms), IP=$ip_address", 'acc_auth.log', 'accounts', 'INFO');
            $_SESSION['public_key'] = $public_key;
            echo json_encode(['status' => 'success', 'message' => 'Registration successful!', 'redirect' => 'profile.php']);
        }
    } catch (Exception $e) {
        log_message("Error: {$e->getMessage()}, IP=$ip_address", 'acc_auth.log', 'accounts', 'ERROR');
