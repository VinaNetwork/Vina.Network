...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['public_key'], $_POST['signature'], $_POST['message'])) {
    ...
    try {
        ...
        if (!preg_match('/with nonce (\w+) at (\d+)/', $message, $matches)) {
            throw new Exception("Thông điệp không chứa nonce hoặc thời gian!");
        }
        $extracted_nonce = $matches[1];
        $timestamp = $matches[2];

        if (!isset($_SESSION['login_nonce']) || $extracted_nonce !== $_SESSION['login_nonce']) {
            throw new Exception("Nonce không hợp lệ!");
        }
        ...
        if (!extension_loaded('bcmath') && !extension_loaded('gmp')) {
            throw new Exception("Vui lòng cài đặt tiện ích mở rộng BC Math hoặc GMP");
        }
        ...
        try {
            ...
            if (strlen($public_key_bytes) !== 32) {
                throw new Exception("Khóa công khai không hợp lệ: Độ dài là " . strlen($public_key_bytes) . " byte, cần 32 byte");
            }
            ...
            try {
                ...
                throw new Exception("Khóa công khai không hợp lệ: Không thuộc đường cong Ed25519 - " . $se->getMessage());
            }
        ...
        try {
            if (!is_string($message_raw) || empty($message_raw)) {
                throw new Exception("Thông điệp không hợp lệ: Rỗng hoặc không phải chuỗi");
            }
            if (!is_string($public_key_bytes) || strlen($public_key_bytes) !== 32) {
                throw new Exception("Khóa công khai không hợp lệ: Độ dài là " . strlen($public_key_bytes) . " byte, cần 32 byte");
            }
            if (!is_string($signature) || strlen($signature) !== 64) {
                throw new Exception("Chữ ký không hợp lệ: Độ dài là " . strlen($signature) . " byte, cần 64 byte");
            }
            ...
            if (!$verified) {
                ...
                if (bin2hex($message_raw) !== bin2hex($message)) {
                    $errors[] = "Mã hóa thông điệp không khớp";
                }
                if (strlen($public_key_bytes) !== 32) {
                ...
                $error_message = "Xác minh chữ ký thất bại: " . (empty($errors) ? "Chữ ký không khớp, vui lòng kết nối lại ví" : implode(", ", $errors));
        ...
        if ($new_csrf_token === false) {
            log_message("Failed to regenerate CSRF token after login, public_key=$short_public_key", 'accounts.log', 'accounts', 'ERROR');
            echo json_encode(['status' => 'error', 'message' => 'Không thể làm mới CSRF token sau khi đăng nhập']);
            exit;
        }
        set_csrf_cookie(); // Update CSRF cookie
        ...
        if ($account) {
            ...
            echo json_encode(['status' => 'success', 'message' => 'Đăng nhập thành công!', 'redirect' => $redirect_url, 'csrf_token' => $new_csrf_token]);
        } else {
            $start_time = microtime(true);
            ...
            $_SESSION['public_key'] = $public_key;
            echo json_encode(['status' => 'success', 'message' => 'Đăng ký thành công!', 'redirect' => $redirect_url, 'csrf_token' => $new_csrf_token]);
        }
    } catch (Exception $e) {
