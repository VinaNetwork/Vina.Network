...
function ensure_session() {
    ...
    try {
        ...
        if (defined('SESSION_STARTED') && session_status() === PHP_SESSION_ACTIVE) {
            log_message("Session already active, session_id=" . session_id() . ", secure=" . ($is_secure ? 'true' : 'false') . ", domain=$domain", 'accounts.log', 'accounts', 'INFO');
            return true;
        }
        ...
        if (!defined('SESSION_STARTED') && session_status() === PHP_SESSION_NONE) {
            ...
            if ($session_started) {
                ...
                log_message("Failed to start session: session_start failed, uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'ERROR');
                return false;
            }
        }

        log_message("Session not active and failed to start, uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'CRITICAL');
        return false;
    } catch (Exception $e) {
        log_message("Error starting session for CSRF: " . $e->getMessage() . ", uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'ERROR');
        return false;
    }
}
...
function generate_csrf_token() {
    try {
        ...
        if (empty($_SESSION[CSRF_TOKEN_NAME])) {
            ...
            log_message("CSRF token generated: " . $_SESSION[CSRF_TOKEN_NAME] . ", created_at=" . $_SESSION[CSRF_TOKEN_NAME . '_created'] . ", uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'INFO');
        }
        return $_SESSION[CSRF_TOKEN_NAME];
    } catch (Exception $e) {
        log_message("Error generating CSRF token: " . $e->getMessage() . ", uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'ERROR');
        return false;
    }
}
...
function validate_csrf_token($token) {
    ...
    if (!ensure_session()) {
        log_message("CSRF token validation failed: session not active, IP=$ip, URI=$uri, Method=$method, AJAX=$is_ajax, User-Agent=$user_agent", 'accounts.log', 'accounts', 'CRITICAL');
        return false;
    }

    if (!isset($_SESSION[CSRF_TOKEN_NAME]) || empty($token)) {
        log_message("CSRF token validation failed: token empty or session token missing, IP=$ip, URI=$uri, Method=$method, AJAX=$is_ajax, User-Agent=$user_agent", 'accounts.log', 'accounts', 'CRITICAL');
        return false;
    }

    // Check TTL
    if (isset($_SESSION[CSRF_TOKEN_NAME . '_created']) && (time() - $_SESSION[CSRF_TOKEN_NAME . '_created']) > CSRF_TOKEN_TTL) {
        log_message("CSRF token validation failed: token expired, provided=$token, created_at=" . $_SESSION[CSRF_TOKEN_NAME . '_created'] . ", IP=$ip, URI=$uri, Method=$method, AJAX=$is_ajax, User-Agent=$user_agent", 'accounts.log', 'accounts', 'WARNING');
        return false;
    }

    if (!hash_equals($_SESSION[CSRF_TOKEN_NAME], $token)) {
        log_message("CSRF token validation failed: provided=$token, expected=" . $_SESSION[CSRF_TOKEN_NAME] . ", IP=$ip, URI=$uri, Method=$method, AJAX=$is_ajax, User-Agent=$user_agent", 'accounts.log', 'accounts', 'WARNING');
        return false;
    }
    
    log_message("CSRF token validated successfully: $token, created_at=" . $_SESSION[CSRF_TOKEN_NAME . '_created'] . ", IP=$ip, URI=$uri, Method=$method, AJAX=$is_ajax, User-Agent=$user_agent", 'accounts.log', 'accounts', 'INFO');
    return true;
}
...
function csrf_protect() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!validate_csrf_token($token)) {
            log_message("CSRF protection triggered: Invalid or expired token, uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'WARNING');
...
function set_csrf_cookie() {
    ...
    log_message("CSRF cookie set: $token, uri=" . ($_SERVER['REQUEST_URI'] ?? 'unknown'), 'accounts.log', 'accounts', 'INFO');
    return true;
}
?>
